...
def engineer_features(dados, window, mode, target, scaler_config=None):
    ...
    if mode == "train":
        ...
        scaler.load_configs(maximo=scaler_config["maximo"], 
                            minimo=scaler_config["minimo"])
    else:
    ...
    day_change_df = create_day_change(dados, window, mode)

    if mode == "train":
        original_vars = ["date", "group"]
    else:
        original_vars = ["date"]

    master_table = consolidate_features(dados[original_vars], "date", 
                                        ibov_lags_df, 
                                        ibov_delta_sign_df,
                                        weekdays_df,
                                        day_change_df)    
...
def create_day_change(df, window, mode):
